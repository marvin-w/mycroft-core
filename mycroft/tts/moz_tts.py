...
class MozillaTTS(TTS):
    """Interface to a locally running Mozilla TTS."""
    def __init__(self, lang, config):
        super(MozillaTTS, self).__init__(
            lang, config, MozillaTTSValidator(self)
        )
        self.url = config['url']
    ...
    def get_tts(self, sentence, wav_file):
        """Generate (remotely) and play mozTTS WAV audio

        Arguments:
            sentence (str): Phrase to synthesize to audio
            wav_file (str): Location to write audio output
        """
        LOG.info("Generating MozTSS for: " + str(sentence))
        try:
            ...
            raise RemoteTTSTimeoutException(
                "Moz TTS server request timed out. Falling back to mimic")
